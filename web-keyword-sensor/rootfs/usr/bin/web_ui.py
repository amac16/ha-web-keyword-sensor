...
<h1>Web Keyword Sensor</h1><p>Manage page checks. Changes are saved immediately.</p><div id="list"></div><div class="card"><h2 id="heading">Add check</h2><form id="form"><input id="id" type="hidden"><label>Name<input id="name" required></label><label>URL<input id="url" type="url" required></label><label>Phrase<input id="phrase" required></label><label>Entity type<select id="entity_type"><option value="binary_sensor">Binary sensor</option><option value="sensor">Sensor</option></select></label><label>Interval<input id="interval" type="number" min="1" max="31536000" value="15" required></label><label>Unit<select id="unit"><option>seconds</option><option selected>minutes</option><option>hours</option><option>days</option><option>weeks</option></select></label><label>Case sensitive<input id="case_sensitive" type="checkbox"></label><label>Verify TLS<input id="verify_ssl" type="checkbox" checked></label><label>Enabled<input id="enabled" type="checkbox" checked></label><div class="actions"><button>Save</button><button type="button" id="cancel" hidden>Cancel</button></div></form></div>
<script>const ids=['name','url','phrase','entity_type','interval','unit','case_sensitive','verify_ssl','enabled'];const $=x=>document.getElementById(x);const esc=x=>String(x).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));async function load(){const x=await (await fetch('./api/checks')).json();$('list').innerHTML=x.map(c=>`<div class="card"><h2>${esc(c.name)}</h2><p>${esc(c.entity_type)} · every ${c.interval} ${esc(c.unit)} · ${c.enabled?'enabled':'disabled'}</p><p>${esc(c.phrase)}<br>${esc(c.url)}</p><button onclick='edit(${JSON.stringify(c)})'>Edit</button> <button class="delete" onclick='del("${esc(c.id)}")'>Delete</button></div>`).join('')||'<p>No checks configured.</p>'}function edit(c){$('id').value=c.id;ids.forEach(k=>$(k)[$(k).type==='checkbox'?'checked':'value']=c[k]);$('heading').textContent='Edit check';$('cancel').hidden=false;scrollTo(0,document.body.scrollHeight)}function reset(){$('form').reset();$('id').value='';$('verify_ssl').checked=true;$('enabled').checked=true;$('heading').textContent='Add check';$('cancel').hidden=true}async function del(id){if(confirm('Delete this check?')){await fetch('./api/checks/'+encodeURIComponent(id),{method:'DELETE'});load()}}$('form').onsubmit=async e=>{e.preventDefault();let c=Object.fromEntries(ids.map(k=>[k,$(k).type==='checkbox'?$(k).checked:$(k).value]));c.id=$('id').value||crypto.randomUUID();c.interval=Number(c.interval);await fetch('./api/checks',{method:$('id').value?'PUT':'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(c)});reset();load()};$('cancel').onclick=reset;load();</script>'''
