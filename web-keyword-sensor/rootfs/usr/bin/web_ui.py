...
async function load(){const r=await fetch('./api/checks');const x=await r.json();checkCache=Object.fromEntries(x.map(c=>[c.id,c]));$('list').innerHTML=x.map(c=>`<div class="card"><h2>${esc(c.name)}</h2><p>${esc(c.entity_type)} · ${c.match_mode==='ai_context'?'AI context':'exact phrase'} · every ${c.interval} ${esc(c.unit)} · ${c.enabled?'enabled':'disabled'}</p><p>${esc(c.context_prompt||c.phrase)}<br>${esc(c.url)}<br><span class="${String(c.auth_status||'').startsWith('Authentication failed')?'auth-failure':''}">${esc(c.auth_status||'Not tested')}</span></p><button class="edit-check" data-id="${esc(c.id)}">Edit</button> <button class="delete delete-check" data-id="${esc(c.id)}">Delete</button></div>`).join('')||'<p>No checks configured.</p>';document.querySelectorAll('.edit-check').forEach(b=>b.onclick=()=>edit(checkCache[b.dataset.id]));document.querySelectorAll('.delete-check').forEach(b=>b.onclick=()=>del(b.dataset.id));loadProfiles()}
