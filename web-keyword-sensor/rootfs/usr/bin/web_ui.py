...
function updateContextFeedback(c){let visible=Boolean(c&&c.id&&c.match_mode==='ai_context');$('context_feedback').classList.toggle('hidden',!visible);$('context_help').classList.toggle('hidden',!visible||c.verification_status!=='failed');$('context_feedback').querySelector('.retest').classList.toggle('hidden',!visible||!['needs_retest','failed'].includes(c.verification_status))}
...
new MutationObserver(showAttention).observe($('list'),{childList:true,subtree:true});
</script>'''
