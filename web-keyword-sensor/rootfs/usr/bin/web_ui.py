...
.more-prompt{color:#1976d2;text-decoration:underline;cursor:pointer;margin-left:4px}
</style>
...
function promptPreview(c){let value=c.context_prompt||c.phrase||'';return value.length>250?esc(value.slice(0,250))+'... <button type="button" class="more-prompt" data-id="'+esc(c.id)+'">more...</button>':esc(value)}
...
async function load(){const r=await fetch('./api/checks');const x=await r.json();checkCache=Object.fromEntries(x.map(c=>[c.id,c]));$('list').innerHTML=x.map(c=>`<div class="card"><h2>${esc(c.name)}</h2><p>${esc(c.entity_type)} · ${c.match_mode==='ai_context'?'AI context':'exact phrase'} · every ${c.interval} ${esc(c.unit)} · ${c.enabled?'enabled':'disabled'}</p><p>Schedule: ${esc(schedule(c))}</p><p>Last Ran: ${esc(dateTime(c.last_ran_at,'Never'))}<br>Next Run: ${esc(dateTime(c.next_run_at,'Pending first run'))}</p><p class="prompt-preview" data-id="${esc(c.id)}">${promptPreview(c)}<br>${esc(c.url)}<br><span class="${String(c.auth_status||'').startsWith('Authentication failed')?'auth-failure':''}">${esc(c.auth_status||'Not tested')}</span></p><button class="edit-check" data-id="${esc(c.id)}">Edit</button> <button class="test-check" data-id="${esc(c.id)}">Test</button> <button class="delete delete-check" data-id="${esc(c.id)}">Delete</button><p class="test-result" id="test-${esc(c.id)}"></p></div>`).join('')||'<p>No checks configured.</p>';document.querySelectorAll('.edit-check').forEach(b=>b.onclick=()=>edit(checkCache[b.dataset.id]));document.querySelectorAll('.test-check').forEach(b=>b.onclick=()=>testCheck(b,b.dataset.id));document.querySelectorAll('.delete-check').forEach(b=>b.onclick=()=>del(b.dataset.id));document.querySelectorAll('.more-prompt').forEach(b=>b.onclick=()=>{let p=b.parentElement;p.innerHTML=esc(checkCache[b.dataset.id].context_prompt||checkCache[b.dataset.id].phrase||'')+'<br>'+esc(checkCache[b.dataset.id].url)+'<br><span class="'+(String(checkCache[b.dataset.id].auth_status||'').startsWith('Authentication failed')?'auth-failure':'')+'">'+esc(checkCache[b.dataset.id].auth_status||'Not tested')+'</span>'});loadProfiles()}
