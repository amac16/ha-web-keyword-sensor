...
<label class="context hidden">AI model<select id="ai_profile_id"></select></label><label class="context hidden" style="grid-column:1/-1">What to look for<textarea id="context_prompt" rows="4" placeholder="Describe the information you want found on this page..."></textarea><small id="context_help" class="context-help context hidden">AI could not find the information you are looking for as described here. Manually check the page where you are looking for this information, and correct the instructions as a necessary.</small><div id="context_feedback" class="context hidden feedback"><button type="button" class="up" onclick="verifyCurrent(true)" title="Verify result">👍</button><button type="button" class="down" onclick="verifyCurrent(false)" title="Result is incorrect">👎</button><button type="button" class="retest hidden" onclick="retestCurrent()">↻ Re-Test?</button></div></label>
...
function updateContextFeedback(c){let visible=Boolean(c&&c.id&&c.match_mode==='ai_context');let verified=c&&c.verification_status==='verified';$('context_feedback').classList.toggle('hidden',!visible);$('context_help').classList.toggle('hidden',!visible||c.verification_status!=='failed');$('context_feedback').querySelector('.up').classList.toggle('hidden',!visible||verified);$('context_feedback').querySelector('.down').classList.toggle('hidden',!visible||verified);$('context_feedback').querySelector('.retest').classList.toggle('hidden',!visible)}
function verifyCurrent(value){let id=$('id').value;if(id)verifyCheck(id,value)}
function retestCurrent(){let id=$('id').value;if(id)testCheck({disabled:false},id,true)}
...
async function verifyCheck(id,verified){let r=await fetch('./api/checks/'+encodeURIComponent(id)+'/verification',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({verified})});if(!r.ok)return;let x=await r.json();checkCache[id]=x.check;await load();updateContextFeedback(x.check)}
async function testCheck(button,id,advanced=false){button.disabled=true;let out=$('test-'+id);out.textContent=advanced?'Running advanced browser re-test...':'Running...';try{let r=await fetch('./api/checks/'+encodeURIComponent(id)+(advanced?'/retest':'/test'),{method:'POST'});let x=await r.json();if(x.ok){out.textContent='Result: state='+x.state+' · matched='+x.attributes.matched;$('feedback-'+id).innerHTML=feedback(id,checkCache[id]?.verification_status==='needs_retest');if(advanced)updateContextFeedback({id,match_mode:'ai_context',verification_status:'needs_retest'})}else out.textContent='Test failed: '+(x.error||'unknown error')}catch(e){out.textContent='Test failed: request error'}finally{button.disabled=false}}
function edit(c){$('id').value=c.id;ids.forEach(k=>$(k)[$(k).type==='checkbox'?'checked':'value']=c[k]??$(k).value);$('auth_mode').value=c.auth_mode||(c.login_url?'basic':'none');$('username').value=c.username||'';$('password').value='';$('password').placeholder=c.login_configured?'******** (stored)':'Password';$('totp_secret').value='';$('advanced_auth').classList.add('hidden');document.querySelectorAll('.day').forEach(x=>x.checked=(c.days||[]).includes(x.value));$('heading').textContent='Edit check';$('cancel').hidden=false;showAuth();showContext();updateContextFeedback(c);scrollTo(0,document.body.scrollHeight)}
function reset(){$('form').reset();$('id').value='';$('time_from').value='00:00';$('time_to').value='23:00';$('auth_mode').value='none';$('match_mode').value='literal';$('password').placeholder='Password';$('advanced_auth').classList.add('hidden');document.querySelectorAll('.day').forEach(x=>x.checked=true);$('heading').textContent='Add check';$('cancel').hidden=true;showAuth();showContext();updateContextFeedback(null)}
...
function showAttention(){document.querySelectorAll('#list .card').forEach(card=>{let id=card.querySelector('.test-check')?.dataset.id;let title=card.querySelector('h2');if(!title||!id)return;let badge=title.querySelector('.needs-attention');if(checkCache[id]?.needs_attention&&!badge){badge=document.createElement('span');badge.className='auth-failure needs-attention';badge.style.cssText='font-size:.55em;margin-left:10px';badge.textContent='Needs Attention';title.append(badge)}else if(!checkCache[id]?.needs_attention&&badge)badge.remove()})}
function styleVerification(){document.querySelectorAll('#list .card p').forEach(p=>{if(p.dataset.verificationStyled)return;let text=p.textContent;if(text.includes('Verification: Verified')||text.includes('Verification: Failed')||text.includes('Verification: Needs re-test')){p.innerHTML=p.innerHTML.replace('Verification: Verified','<span class="verification verified">Verification: Verified</span>').replace('Verification: Failed','<span class="verification failed">Verification: Failed</span>').replace('Verification: Needs re-test','<span class="verification failed">Verification: Needs re-test</span>');p.dataset.verificationStyled='true'}})}
new MutationObserver(()=>{showAttention();styleVerification()}).observe($('list'),{childList:true,subtree:true});
</script>'''
